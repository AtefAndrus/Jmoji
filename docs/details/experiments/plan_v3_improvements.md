# 実験計画: v3データセットでの学習改善

> **注**: 本計画に基づく実験は完了しました。結果は [v3_improvements.md](v3_improvements.md) を参照してください。

## 概要

本ドキュメントは、v3データセット（5,000件）での学習結果（[v3_5000samples.md](v3_5000samples.md)）を受けて、Soft Mode Collapseを解消するための実験計画を記述する。

**目標**: Top5絵文字への偏りを解消し、Jaccard > 0.10を達成する

**結果**: 目標未達。最良はtop100でJaccard 0.058（+29%）、多様性19.8%。データ密度の向上が次の課題。

---

## 背景

### 現状の問題（v3_baseline）

| 指標 | 値 | 評価 |
|------|-----|------|
| Average Jaccard | 0.0449 | 非常に低い |
| Exact Match Rate | 0.0000 | 完全一致なし |
| Micro F1 | 0.0766 | ほぼランダム |

**Soft Mode Collapse**: 出力がTop5絵文字（🎵、📚、🎤、🎉、🏆）に集中

### 原因の仮説

1. **学習率が高すぎる**: 頻出クラスに早期収束
2. **絵文字種類が多すぎる**: 785種類に対して4,000件では不足（1絵文字あたり平均5件）
3. **クロスエントロピー損失の限界**: クラス不均衡に弱い

---

## 実験計画

### 方針

- 変数を1つずつ変えて効果を検証（科学的アプローチ）
- v3_baseline を基準として比較
- 効果が確認できたら組み合わせ

### 実験一覧

| # | 実験名 | 変更点 | Jaccard | 状態 |
|---|--------|--------|---------|------|
| 0 | v3_baseline | - | 0.045 | 完了 |
| 1 | v3_lr1e-4 | learning_rate: 3e-4→1e-4 | 0.048 | 完了 |
| 2 | v3_top100 | 絵文字をTop-100に制限 | **0.058** | 完了（最良） |
| 3 | v3_lr1e-4_top100 | 1+2の組み合わせ | 0.000 | 完了（失敗） |
| 4 | v3_focal_top100 | Focal Loss + Top-100 | 0.058 | 完了 |

---

## 各実験の詳細

### Exp1: 学習率調整 (v3_lr1e-4)

**仮説**: 学習率が高すぎて頻出クラスに早期収束している

| 項目 | ベースライン | 変更後 |
|------|-------------|--------|
| learning_rate | 3e-4 | 1e-4 |

**根拠**:

- T5の推奨学習率は1e-4〜3e-4
- 不均衡データでは低い学習率が安定する傾向

**実装**: ノートブックのCONFIG変更のみ（1行）

```python
CONFIG = {
    ...
    "learning_rate": 1e-4,  # 3e-4 -> 1e-4
    ...
}
```

**期待効果**: より慎重な最適化でmode collapse緩和

---

### Exp2: Top-100絵文字制限 (v3_top100)

**仮説**: 785種類は多すぎる。学習対象を絞ることで各絵文字のサンプル数を増加

| 項目 | ベースライン | 変更後 |
|------|-------------|--------|
| 絵文字種類 | 785 | 100 |
| サンプル数 | 5,000 | 約2,500〜3,500（推定） |
| 1絵文字あたりサンプル | 約5件 | 約25〜35件 |

**根拠**:

- 実用上よく使われる絵文字はTop-100程度
- 学習データの密度を上げることで汎化性能向上

**実装**: ノートブックにフィルタリングセルを追加

```python
# Top-100絵文字を特定
top_100_emojis = set(e for e, _ in emoji_counts.most_common(100))
print(f"Top-100 emojis: {len(top_100_emojis)}")

# フィルタリング（全絵文字がTop-100に含まれるサンプルのみ）
filtered_samples = [
    s for s in samples
    if all(e in top_100_emojis for e in s["emoji_string"].split())
]
print(f"Filtered samples: {len(filtered_samples)} / {len(samples)}")

# フィルタ後のサンプルを使用
samples = filtered_samples
```

**期待効果**: 学習対象が絞られ、各絵文字の学習機会が増加

---

### Exp3: 組み合わせ (v3_lr1e-4_top100)

**仮説**: Exp1とExp2の相乗効果

| 項目 | 値 |
|------|-----|
| learning_rate | 1e-4 |
| 絵文字種類 | Top-100 |

**実施条件**: Exp1またはExp2で改善が見られた場合

**実装**: Exp1 + Exp2の変更を両方適用

---

### Exp4: Focal Loss (v3_focal)

**仮説**: クロスエントロピーは頻出クラスに偏りやすい。Focal Lossで低頻度クラスの学習を促進

**Focal Lossの式**:

```text
FL(p_t) = -α_t * (1 - p_t)^γ * log(p_t)
```

| パラメータ | 値 | 説明 |
|-----------|-----|------|
| γ (gamma) | 2.0 | focusing parameter（標準値） |
| α (alpha) | 1.0 | class weight（初期は均等） |

**根拠**:

- 物体検出（RetinaNet）でクラス不均衡問題を解決した手法
- 簡単な例（高確率で正解）の損失を下げ、難しい例に集中

**実装**: `src/models/t5_trainer.py`にカスタムTrainerを追加

```python
class FocalLossTrainer(Trainer):
    def __init__(self, gamma: float = 2.0, **kwargs):
        super().__init__(**kwargs)
        self.gamma = gamma

    def compute_loss(self, model, inputs, return_outputs=False, **kwargs):
        labels = inputs.pop("labels")
        outputs = model(**inputs)
        logits = outputs.logits

        # Focal Loss計算
        ce_loss = F.cross_entropy(
            logits.view(-1, logits.size(-1)),
            labels.view(-1),
            reduction='none',
            ignore_index=-100
        )
        pt = torch.exp(-ce_loss)
        focal_loss = ((1 - pt) ** self.gamma * ce_loss).mean()

        return (focal_loss, outputs) if return_outputs else focal_loss
```

**実施条件**: Exp1-3で十分な改善が見られない場合

---

## 成功基準

| 指標 | ベースライン | 目標 | 備考 |
|------|-------------|------|------|
| Average Jaccard | 0.045 | > 0.10 | 2倍以上の改善 |
| Top5以外の出力割合 | 〜0% | > 30% | 多様性指標 |
| Exact Match | 0% | > 1% | 完全一致の出現 |

### 多様性指標の計算方法

```python
def diversity_ratio(predictions, top_n_emojis):
    """Top-N絵文字以外の出力割合を計算"""
    total = 0
    non_top_n = 0
    for pred in predictions:
        emojis = pred.split()
        total += len(emojis)
        non_top_n += sum(1 for e in emojis if e not in top_n_emojis)
    return non_top_n / total if total > 0 else 0
```

---

## 実施順序

```text
Exp1 (lr1e-4)
    │
    ├─ 改善あり → Exp3 (lr1e-4 + top100)
    │
    └─ 改善なし → Exp2 (top100)
                      │
                      ├─ 改善あり → Exp3
                      │
                      └─ 改善なし → Exp4 (focal)
```

**推定所要時間**: 各実験30分〜1時間（Colab L4 GPU）

---

## 必要な実装タスク

| タスク | 対象ファイル | 優先度 | 状態 |
|--------|-------------|--------|------|
| Exp1用CONFIG変更 | notebooks/train_t5_colab.py | 高 | 完了 |
| Top-100フィルタリングセル追加 | notebooks/train_t5_colab.py | 高 | 完了 |
| FocalLossTrainer実装 | src/models/t5_trainer.py | 中 | 完了 |
| 多様性指標の追加 | src/evaluation/metrics.py | 中 | 完了 |

---

## 参考文献

- [Focal Loss for Dense Object Detection](https://arxiv.org/abs/1708.02002) (Lin et al., 2017)
- [Exploring the Limits of Transfer Learning with a Unified Text-to-Text Transformer](https://arxiv.org/abs/1910.10683) (Raffel et al., 2019)

---

## 関連ドキュメント

- [v3_improvements.md](v3_improvements.md): **本計画の実験結果**
- [v3_5000samples.md](v3_5000samples.md): v3ベースラインの実験結果
- [v1_1000samples.md](v1_1000samples.md): v1での実験（完全mode collapse）
- [generation_v3.md](../datasets/generation_v3.md): v3データセットの品質改善
